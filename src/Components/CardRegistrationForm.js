...
const CardRegistrationFrom = (props) => {
  ...
  const {
    ...
    valueBlurChangeHandler: nameBlurHandler,
    resetInput: resetNameInput,
  } = useInput(textInputValidator);
  const {
    value: enteredCardNumber,
    isValid: cardIsValid,
    hasError: cardHasError,
    valueChangeHandler: CardNumberChangeHandler,
    valueBlurChangeHandler: CardNumberBlurHandler,
    resetInput: resetCardNumberInput,
  } = useInput(cardNumberValidator);
  const {
    value: enteredCCV,
    isValid: CCVIsValid,
    hasError: CCVHasError,
    valueChangeHandler: CCVChangeHandler,
    valueBlurChangeHandler: CCVBlurHandler,
    resetInput: resetCCVInput,
  } = useInput(CCVValidator);
  const {
    value: enteredDate,
    isValid: DateIsValid,
    hasError: DateHasError,
    valueChangeHandler: DateChangeHandler,
    valueBlurChangeHandler: DateBlurHandler,
  ...
  const nameInputClasses = nameHasError
    ? formControlInvalidStyles
    : formControlStyles;
  const cardNumberClasses = cardHasError
    ? formControlInvalidStyles
    : formControlStyles;
    ...
    <form onSubmit={formSubmitHandler}>
      ...
      <div className={styles["wrapper-div"]}>
        <div className={cardNumberClasses}>
          ...
          />
          {cardHasError && (
            <p className={errorText}>{creditCardErrorMessage}</p>
          )}
        </div>
        <div className={CCVClasses}>
          ...
          <input
            type="number"
            id="ccv"
            maxLength={maxLengthCCV}
            onChange={CCVChangeHandler}
            onBlur={CCVBlurHandler}
            value={enteredCCV}
          />
